---
title: Aspose.PSD for .NET 20.11 - 릴리스 노트
type: docs
weight: 20
url: /ko/net/aspose-psd-for-net-20-11-release-notes/
---

{{% alert color="primary" %}} 

이 페이지에는 [Aspose.PSD for .NET 20.11](https://www.nuget.org/packages/Aspose.PSD/)의 릴리스 노트가 포함되어 있습니다.

{{% /alert %}} 

|**키**|**요약**|**카테고리**|
| :- | :- | :- |
|PSDNET-713|Aspose.PSD는 PSD를 파일로 저장하지 않고 다른 색 모드/비트 깊이로 변환할 수 없음|기능|
|PSDNET-754|PSD 이미지에서 스마트 객체 레이어를 복사하는 기능 추가|기능|
|PSDNET-267|레이어 효과가 있는 PSD 파일을 로드하고 저장할 때 예외 발생|버그|
|PSDNET-579|"Image.LoadRawData" 메서드가 NullPointer 예외를 throw 함|버그|
|PSDNET-741|파일을 열려고 시도할 때 ImageLoadException이 throw 됨|버그|
|PSDNET-744|Aspose.PSD 20.10: Psd를 로드할 수 없음|버그|
...
## **사용 예시:**
**PSDNET-267. 레이어 효과가 있는 PSD 파일을 로드하고 저장할 때 예외 발생**
{{< highlight csharp >}}
...
{{< /highlight >}}
**PSDNET-579. "Image.LoadRawData" 메서드가 NullPointer 예외를 throw 함**
{{< highlight csharp >}}
...
{{< /highlight >}}
**PSDNET-713. Aspose.PSD는 PSD를 파일로 저장하지 않고 다른 색 모드/비트 깊이로 변환할 수 없음**
{{< highlight csharp >}}
            string dataDir = "PSDNET713_1\\";
            string outputDir = dataDir + "output\\";

            // 이 예제들은 PSD 이미지 형식을 다른 색 모드/비트 깊이로 변환하는 방법을 보여줍니다.
...
{{< /highlight >}}
**PSDNET-741. 파일을 열려고 시도할 때 ImageLoadException이 throw 됨**
{{< highlight csharp >}}
...
{{< /highlight >}}
**PSDNET-744. Aspose.PSD 20.10: Psd를 로드할 수 없음**
{{< highlight csharp >}}         
...
{{< /highlight >}}
**PSDNET-754. PSD 이미지에서 스마트 객체 레이어를 복사하는 기능 추가**
{{< highlight csharp >}}
            ...
            void ExampleOfCopingSmartObjectLayer(string fileName)
            {
                ...
                using (PsdImage image = (PsdImage)Image.Load(filePath))
                {
                    ...
                    {
                        // 내장 스마트 객체 이미지를 반전시킵니다.
                        InvertImage(innerImage);

                        // PSD 레이어에서 내장 스마트 객체 이미지를 교체합니다.
                        smartObjectLayer.ReplaceContents(innerImage);
                    }

                    // 복제된 레이어는 원래 스마트 객체와 이미지를 공유하며 명시적으로 업데이트해야 합니다.
