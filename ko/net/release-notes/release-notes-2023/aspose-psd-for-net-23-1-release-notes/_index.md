---
title: Aspose.PSD for .NET 23.1 - 릴리스 노트
type: docs
weight: 120
url: /ko/net/aspose-psd-for-net-23-1-release-notes/
---

{{% alert color="primary" %}}

이 페이지에는 [Aspose.PSD for .NET 23.1](https://www.nuget.org/packages/Aspose.PSD/)의 릴리스 노트가 포함되어 있습니다.

{{% /alert %}}

|**키**|**요약**|**카테고리**|
| :- | :- | :- |
|PSDNET-401|vstkResource 지원|기능|
|PSDNET-1346|IText 인터페이스에 편집 가능한 BaselineDirection/IsStandardVerticalRomanAlignmentEnabled 속성 추가|기능|
|PSDNET-181|PSD를 JPEG로 제대로 변환하지 않음|버그|
|PSDNET-958|대용량 파일에서 PSB를 PDF로 변환하지 못하는 문제|버그|
|PSDNET-1171|조정 레이어의 클리핑 마스크 처리 추가|버그|
|PSDNET-1252|전체 이미지 크기 조정 후 특정 레이어 크기 조정 시 Aspose.PSD가 레이어 저장 시 예외를 발생시킴|버그|
...
**PSDNET-181. PSD를 JPEG로 제대로 변환하지 않음**
...
**PSDNET-958. 대용량 파일에서 PSB를 PDF로 변환하지 못하는 문제**
...
**PSDNET-1171. 조정 레이어의 클리핑 마스크 처리 추가**
...
**PSDNET-1252. 전체 이미지 크기 조정 후 특정 레이어 크기 조정 시 Aspose.PSD가 레이어 저장 시 예외를 발생시킴**
...
**PSDNET-1346. IText 인터페이스에 편집 가능한 BaselineDirection/IsStandardVerticalRomanAlignmentEnabled 속성 추가**

{{< highlight csharp >}}
// 다음 코드는 새 IsStandardVerticalRomanAlignmentEnabled 속성을 편집할 수 있는 능력을 보여줍니다.
// 현재 렌더링에는 영향을 주지 않지만 속성 값을 편집할 수만 있습니다.
...
using (var image = (PsdImage)Image.Load(src))
{
    ...
    {
        throw new Exception("IsStandardVerticalRomanAlignmentEnabled 속성 값의 잘못된 읽기");
    }
