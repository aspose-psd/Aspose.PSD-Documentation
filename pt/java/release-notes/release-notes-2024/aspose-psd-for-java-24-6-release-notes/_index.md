---
title: Notas de Lançamento do Aspose.PSD para Java 24.6
type: docs
weight: 40
url: /pt/java/aspose-psd-for-java-24-6-release-notes/
---

{{% alert color="primary" %}} Esta página contém as notas de lançamento para o [Aspose.PSD para Java 24.6](https://downloads.aspose.com/psd/java/new-releases/aspose.psd-for-java-24.6/) {{% /alert %}}

| **Chave**    | **Resumo**                                                                                                     | **Categoria** |
|:------------|:---------------------------------------------------------------------------------------------------------------|:--------------|
| PSDJAVA-628 | Implementar suporte à camada do mapa de degradê                                                                | Recurso      |
| PSDJAVA-629 | [Formato AI] Adicionar suporte a Metadados XPacket ao Formato AI                                                | Recurso      |
| PSDJAVA-630 | Implementar tipos de warp Inflate, Squeeze e Twist                                                             | Recurso      |
| PSDJAVA-631 | Modos Rgb e Lab não podem conter menos de 3 canais e mais de 4 canais no arquivo com Camadas de ArtBoard       | Correção     |
| PSDJAVA-632 | A parte superior da área de processamento deve ser positiva. (Parâmetro 'areaToProcess') no processamento de arquivo específico | Correção     |
| PSDJAVA-633 | A imagem expandida sobre o canvas é cortada após a salvamento. Os dados são perdidos mas a visualização está correta  | Correção     |
...
**PSDJAVA-628. Implementar suporte à camada do mapa de degradê**
...
{{< highlight java >}}
    ...
    public static void main(String[] args) {
        ...
        try (PsdImage im = (PsdImage) Image.load(sourceFile)) {
            // Adicionar camada de ajuste de mapa de degradê.
...
**PSDJAVA-629. [Formato AI] Adicionar suporte a Metadados XPacket ao Formato AI**
...
{{< highlight java >}}
    ...
    public static void main(String[] args) {
        ...
        try (AiImage image = (AiImage) Image.load(sourceFile)) {
            // Metadados Xmp foram adicionados.
            var xmpMetaData = image.getXmpData();

            assertIsNotNull(xmpMetaData);

            // Agora podemos ter acesso aos Pacotes Xmp dos arquivos AI.
    ...
    static void assertIsNotNull(Object testObject) {
        if (testObject == null) {
            throw new RuntimeException("O objeto de teste é nulo.");
        }
    }

{{< /highlight >}}

**PSDJAVA-630. Implementar tipos de warp Inflate, Squeeze e Twist**
...
**PSDJAVA-631. Modos Rgb e Lab não podem conter menos de 3 canais e mais de 4 canais no arquivo com Camadas de ArtBoard**
...
**PSDJAVA-632. A parte superior da área de processamento deve ser positiva. (Parâmetro 'areaToProcess') no processamento de arquivo específico**
...
{{< highlight java >}}
    ...
    try (PsdImage image = (PsdImage) PsdImage.load(sourceFile, psdLoadOptions)) {
        image.save(outputFile);
        // Não deve ocorrer exceção
    }

{{< /highlight >}}

**PSDJAVA-633. A imagem expandida sobre o canvas é cortada após a salvamento. Os dados são perdidos mas a visualização está correta**
