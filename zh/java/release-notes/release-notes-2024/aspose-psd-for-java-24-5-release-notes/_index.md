---
title: Aspose.PSD for Java 24.5 - 发布说明
type: docs
weight: 40
url: /zh/java/aspose-psd-for-java-24-5-release-notes/
---

{{% alert color="primary" %}} 本页包含 [Aspose.PSD for Java 24.5](https://downloads.aspose.com/psd/java/new-releases/aspose.psd-for-java-24.5/) 的发布说明 {{% /alert %}}

| **关键字**  | **概要**                                                                                                               | **类别**  |
|:------------|:-----------------------------------------------------------------------------------------------------------------------|:----------|
| PSDJAVA-617 | [AI 格式] 添加对带有 EPSF 标头的 AI 文件处理的支持                                                                     | 功能       |
| PSDJAVA-620 | PSD 文件预览中半透明处理错误                                                                                            | 错误       |
| PSDJAVA-621 | 形状图层的呈现部分不正确                                                                                               | 错误       |
| PSDJAVA-622 | 保存 PSD 文件时出现异常，大小超过 200 MB 和大尺寸时存在内存使用问题                                                     | 错误       |
| PSDJAVA-623 | 从 23.7 更新到 24.3 后，保存为 PDF 时图像保存失败异常                                                                    | 错误       |
| PSDJAVA-624 | 修复获取中文字体信息记录方法的问题                                                                                       | 错误       |
...
**PSDJAVA-617. [AI 格式] 添加对带有 EPSF 标头的 AI 文件处理的支持**
    ...
    public static void main(String[] args) {
        ...
        try (PsdImage im = (PsdImage) Image.load(sourceFile)) {
            ...
            secondBezierKnotRecord.setPoints(new Point[]{
                    ...
                    pointFToResourcePoint(
                            new PointF(100, 490),
                            shapeLayer.getContainer().getSize()), // 锚点
                    pointFToResourcePoint(
                            new PointF(150, 490),
                            shapeLayer.getContainer().getSize())
            });
...
**PSDJAVA-622. 保存 PSD 文件时出现异常，大小超过 200 MB 和大尺寸时存在内存使用问题**
...
**PSDJAVA-623. 从 23.7 更新到 24.3 后，保存为 PDF 时图像保存失败异常**
