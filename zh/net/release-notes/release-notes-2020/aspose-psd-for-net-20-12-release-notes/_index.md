---
title: Aspose.PSD for .NET 20.12 - 发行说明
type: docs
weight: 10
url: /zh/net/aspose-psd-for-net-20-12-release-notes/
---

{{% alert color="primary" %}} 

本页包含[Aspose.PSD for .NET 20.12](https://www.nuget.org/packages/Aspose.PSD/)的发行说明

{{% /alert %}} 

|**关键**|**摘要**|**类别**|
| :- | :- | :- |
|PSDNET-757|支持将图层转换为智能对象图层|功能|
|PSDNET-764|如果尝试添加PSB文件，SmartObjectLayer.ReplaceContents方法会引发NullReferenceException|错误|
|PSDNET-773|如果图层大于画布，则CMYK 8位和CMYK 16位图像的渲染不正确|错误|
|PSDNET-782|保存的PSB文件可以使用我们的API打开，但无法使用Photoshop打开|错误|
|PSDNET-783|如果尝试在具有共享数据源的特定PSD中更改智能图层，则会出现异常|错误|
|PSDNET-172|将FileFormatVersion重命名为PsdVersion以避免混淆|增强功能|
|PSDNET-620|从Aspose.PSD .NET中删除Compact框架配置|增强功能|
|PSDNET-765|在尝试使用SmartObjectLayers打开PSB文件时PsdImageException: 未知资源标头|增强功能|
|PSDNET-798|将矢量蒙版类的层次结构移动到核心命名空间，以确保Aspose.PSD和Aspose.Imaging产品的一致性|增强功能|
...
- F:Aspose.PSD.FileFormats.Psd.Layers.BlendMode