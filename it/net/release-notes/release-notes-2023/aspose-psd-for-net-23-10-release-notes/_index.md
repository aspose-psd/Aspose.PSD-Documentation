---
title: Note sulla versione di Aspose.PSD per .NET 23.10
type: docs
weight: 30
url: /it/net/aspose-psd-per-net-23-10-note-sulla-versione/
---

{{% alert color="primary" %}}

Questa pagina contiene le note sulla versione di [Aspose.PSD per .NET 23.10](https://www.nuget.org/packages/Aspose.PSD/)

{{% /alert %}}

| **Chiave**     | **Sommario**                                                                                                                | **Categoria** |
|:------------|:------------------------------------------------------------------------|:--------|
| PSDNET-692 | Supporto della direzione del testo verticale | Funzionalità |
| PSDNET-1402 | Utilizzo delle impostazioni del tratto dalle risorse vstk nel rendering del tratto della forma | Funzionalità |
| PSDNET-1607 | Implementazione del rendering dell'area interna delle forme a tratto | Funzionalità |
| PSDNET-1644 | Non ridisegnare il livello della forma se non è stato modificato | Funzionalità |
| PSDNET-1696 | [Formato AI] Aggiunta del supporto per la lettura dell'intestazione dai file AI basati su PDF | Funzionalità |
| PSDNET-1560 | Vari modi per impostare la risoluzione del file Psd non funzionano | Bug |
| PSDNET-1728 | FontSettings.SetFontsFolders non funziona o Aspose.PSD utilizza un font non corretto | Bug |
| PSDNET-1739 | Regression. Correggere l'eccezione Null reference che si verifica su PsdImage.Save quando è presente un livello di forma | Bug |
...
**PSDNET-692. Supporto della direzione del testo verticale**
...
**PSDNET-1402. Utilizzo delle impostazioni del tratto dalle risorse vstk nel rendering del tratto della forma**
...
**PSDNET-1607. Implementazione del rendering dell'area interna delle forme a tratto**
...
**PSDNET-1644. Non ridisegnare il livello della forma se non è stato modificato**
...
using (PsdImage image = (PsdImage)Image.Load(sourceFile))
{
    ...
    // Non utilizzare l'aggiornamento del livello della forma prima del salvataggio

    image.Save(outputFile);

    // Verifica il rendering del file salvato
}

// Verifica che i dati del livello di forma rimangano invariati.
using (PsdImage image = (PsdImage)Image.Load(outputFile))
{
    ShapeLayer shapeLayer = (ShapeLayer)image.Layers[1];
    ColorFillSettings fillSettings = (ColorFillSettings)shapeLayer.Fill;
...
**PSDNET-1696. [Formato AI] Aggiunta del supporto per la lettura dell'intestazione dai file AI basati su PDF**
...
void AssertIsNotNull(object expected)
{
    if (expected == null)
    {
        throw new Exception("L'oggetto è nullo.");
    }
}
...
**PSDNET-1560. Vari modi per impostare la risoluzione del file Psd non funzionano**
...
**PSDNET-1728. FontSettings.SetFontsFolders non funziona o Aspose.PSD utilizza un font non corretto**
...
**PSDNET-1739. Regression. Correggere l'eccezione Null reference che si verifica su PsdImage.Save quando è presente un livello di forma**
...
{
    // Dovrebbe caricare senza eccezioni

    ShapeLayer shapeLayer = (ShapeLayer)im.Layers[1];

    if (shapeLayer.RawDataSettings == null)
    {
        throw new Exception("RawDataSettings non dovrebbe essere nullo.");
    }
}
{{< /highlight >}}