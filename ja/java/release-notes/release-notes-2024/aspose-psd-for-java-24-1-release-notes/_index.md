---
title: Aspose.PSD for Java 24.1 - リリースノート
type: docs
weight: 40
url: /ja/aspose-psd-for-java-24-1-release-notes/
---

{{% alert color="primary" %}} このページには、[Aspose.PSD for Java 24.1](https://downloads.aspose.com/psd/java/new-releases/aspose.psd-for-java-24.1/) のリリースノートが含まれています {{% /alert %}}

| **キー**     | **サマリー**                                                                                  | **カテゴリー** |
|:------------|:-----------------------------------------------------------------------------------------------|:-------------|
| PSDJAVA-576 | [AIフォーマット] 複数ページのAI画像の基本的な処理を追加                                        | 機能         |
| PSDJAVA-579 | ワープテキスト効果がテキストに適用されない                                                      | バグ         |
| PSDJAVA-580 | 特定のファイルでマスクが不正確にレンダリングされる                                              | バグ         |
| PSDJAVA-581 | Aspose.PSD.FontParsing.OpenType.Serialization.OpenTypeFontInfo.ctor における NullReferenceException | バグ         |
| PSDJAVA-582 | [AIフォーマット] AiExporterUtils におけるメモリ使用量の修正                                       | バグ         |
...
**PSDJAVA-576. [AIフォーマット] 複数ページのAI画像の基本的な処理を追加**
...
{{< highlight java >}}
        ...
        try (AiImage image = (AiImage) Image.load(sourceFile)) {
            ...
            // したがって、このプロパティを変更せずにAI画像を保存すると、最初のページがレンダリングされて保存されます。
            image.save(firstPageOutputPng, new PngOptions());

            // アクティブページインデックスを2番目のページに変更します。
            image.setActivePageIndex(1);

            // AI画像の2番目のページをPNG画像として保存します。
            image.save(secondPageOutputPng, new PngOptions());

            // アクティブページインデックスを3番目のページに変更します。
            image.setActivePageIndex(2);

            // AI画像の3番目のページをPNG画像として保存します。
            image.save(thirdPageOutputPng, new PngOptions());
        }

{{< /highlight >}}

**PSDJAVA-579. ワープテキスト効果がテキストに適用されない**
...
**PSDJAVA-580. 特定のファイルでマスクが不正確にレンダリングされる**
...
**PSDJAVA-581. Aspose.PSD.FontParsing.OpenType.Serialization.OpenTypeFontInfo.ctor における NullReferenceException**
...
**PSDJAVA-582. [AIフォーマット] AiExporterUtils におけるメモリ使用量の修正**
