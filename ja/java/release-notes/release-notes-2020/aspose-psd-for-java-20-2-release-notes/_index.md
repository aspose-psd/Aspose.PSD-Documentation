---
title: Aspose.PSD for Java 20.2 - リリースノート
type: docs
weight: 20
url: /ja/java/aspose-psd-for-java-20-2-release-notes/
---

{{% alert color="primary" %}} このページには、[Aspose.PSD for Java 20.2](https://downloads.aspose.com/psd/java/new-releases/aspose.psd-for-java-20.2/) のリリースノートが含まれています。{{% /alert %}}

|**キー**|**概要**|**カテゴリ**|
| :- | :- | :- |
|PSDJAVA-96|テキストレイヤー内の異なる色のテキストをレンダリングする機能を改善|機能|
|PSDJAVA-97|clbl リソースのサポート (レイヤーリソースにブレンドクリッピング要素に関する情報を含む)|機能|
|PSDJAVA-100|blwh リソースのサポート (ブラック＆ホワイト調整レイヤーデータを含むリソース)|機能|
|PSDJAVA-101|レイヤーグループを Jpeg/Png/Tiff/Gif/Bmp/Jpeg2000/Psd/Psb/Pdf へエクスポートする機能|機能|
|PSDJAVA-105|lspf リソースのサポート (レイヤー保護設定に関する情報を含む)|機能|
|PSDJAVA-106|infx リソースのサポート (内部要素のブレンドに関するデータを含む)|機能|
|PSDJAVA-99|PsdImage およびレイヤーのリファクタリングを行い、変換動作を変更する(Layer のみを変換した場合の正しいリサイズ/回転/クロップなど)|強化|
|PSDJAVA-98|特定のグローバリゼーション設定で AI 画像ラスタ画像が開けない問題|バグ|
|PSDJAVA-102|レイヤーに FlipRotate 操作を行った後、PSD 画像が読み込めなくなる問題|バグ|
|PSDJAVA-103|PSD ファイルの読み込み中に System.ArgumentException|バグ|
|PSDJAVA-104|レイヤーのみに変換メソッドを使用した後、保存されたレイヤーの境界またはマスクが不正確|バグ|

# **公開 API 変更**
# **この API は一時的に無効化され、次のリリースで有効になります:**
...
` `**適切な依存関係がある場合は、間に v19.4 を使用してください。**
# **追加された API:**
...
- M:com.aspose.psd.fileformats.psd.layers.adjustmentlayers.BlackWhiteAdjustmentWhiteAdjustmentLayer.setBlues(int)
