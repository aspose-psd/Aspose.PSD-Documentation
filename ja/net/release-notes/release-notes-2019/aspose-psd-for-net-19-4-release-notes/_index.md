---
title: Aspose.PSD for .NET 19.4 - リリースノート
type: docs
weight: 90
url: /ja/net/aspose-psd-for-net-19-4-release-notes/
---

{{% alert color="primary" %}} 

このページには、Aspose.PSD for .NET 19.4のリリースノートが含まれています。

{{% /alert %}} 

|**キー**|**サマリー**|**カテゴリ**|
| :- | :- | :- |
|PSDNET-87|JPEG/PNGなどの画像ファイルを直接ロードせずにPsdImageにロードする機能を追加する（Aspose.PSDではサポートされていない）|機能|
|PSDNET-120|16ビット/チャンネルのRGBカラーモード（1色あたり64ビット）のサポート|機能|
|PSDNET-108|レイヤーベクターマスクとテキストレイヤーのカスタムフリップローテーションのサポート|機能|
|PSDNET-99|全アジア文字が適切にレンダリングされない|バグ|
|PSDNET-116|\r\n記号が2重の改行として解釈されるのは間違っている|バグ|
|PSDNET-117|テキストレイヤーを改行を含む文字列で更新すると、PSDファイルが読み取れなくなる|バグ|
|PSDNET-118|タブ記号を含む文字列でテキストレイヤーを更新すると、処理が例外で失敗する|バグ|
...
## **使用例:**
**PSDNET-87. JPEG/PNGなどの画像ファイルを直接ロードせずにPsdImageにロードする機能を追加する（Aspose.PSDではサポートされていない）**
...
**PSDNET-120. 16ビット/チャンネルのRGBカラーモード（1色あたり64ビット）のサポート**

{{< highlight java >}}
  // 16ビット/チャンネルのRGBカラーモード（1色あたり64ビット）のサポート
...
// ファイルをPhotoshopで問題なく開く必要があります    

using(Image image = Image.Load(outputFilePathPsd)) {}  

{{< /highlight >}}

**PSDNET-108. レイヤーベクターマスクとテキストレイヤーのカスタムフリップローテーションのサポート**

{{< highlight java >}}
 // PSDにおいてRotateFlip操作が期待通りに機能しない
...
**PSDNET-99. 全アジア文字が適切にレンダリングされない**

[**添付の例をチェックしてください**](attachments/86278147/86343681.cs)

**PSDNET-116. \r\n記号が2重の改行として解釈されるのは間違っている**
...
// ファイルは例外なく開かれ、Photoshopで読み取れる必要があり、各行の間に3つの改行が含まれている必要があります

using(Image image = Image.Load(exportPath)) {}
{{< /highlight >}}

**PSDNET-117. テキストレイヤーを改行を含む文字列で更新すると、PSDファイルが読み取れなくなる**

{{< highlight java >}}
 // テキストレイヤーを改行を含む文字列で更新すると、PSDファイルが読み取れなくなります。
...
// ファイルは例外なく開かれ、Photoshopで読み取れる必要があります

using(Image image = Image.Load(exportPath)) {}
{{< /highlight >}}

**PSDNET-118. タブ記号を含む文字列** 
