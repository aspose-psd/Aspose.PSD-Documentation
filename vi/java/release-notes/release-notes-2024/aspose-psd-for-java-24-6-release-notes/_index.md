---
title: Aspose.PSD cho Java 24.6 - Ghi chú Phát hành
type: docs
weight: 40
url: /vi/java/aspose-psd-cho-java-24-6-ghi-chu-phat-hanh/
---

{{% alert color="primary" %}} Trang này chứa ghi chú phát hành cho [Aspose.PSD cho Java 24.6](https://downloads.aspose.com/psd/java/new-releases/aspose.psd-for-java-24.6/) {{% /alert %}}

| **Khóa**     | **Tóm tắt**                                                                                                      | **Danh mục** |
|:------------|:-----------------------------------------------------------------------------------------------------------------|:-------------|
| PSDJAVA-628 | Triển khai hỗ trợ lớp bản đồ Gradient                                                                             | Tính năng    |
| PSDJAVA-629 | [Định dạng AI] Thêm hỗ trợ cho XPacket Metadata cho Định dạng AI                                                 | Tính năng    |
| PSDJAVA-630 | Triển khai loại Warp Inflate, Squeeze và Twist                                                                   | Tính năng    |
| PSDJAVA-631 | Chế độ Rgb và Lab không thể chứa ít hơn 3 kênh và nhiều hơn 4 kênh trong tập tin với Lớp ArtBoard             | Lỗi          |
| PSDJAVA-632 | Khu vực xử lý phía trên phải là số dương. (Tham số 'areaToProcess') trong quá trình xử lý tập tin cụ thể       | Lỗi          |
| PSDJAVA-633 | Hình ảnh mở rộng trên bảng vẽ bị cắt sau khi lưu. Dữ liệu bị mất nhưng xem trước đúng                       | Lỗi          |
...
**PSDJAVA-628. Triển khai hỗ trợ lớp bản đồ Gradient**
...
{{< highlight java >}}
    ...
    public static void main(String[] args) {
        ...
        try (PsdImage im = (PsdImage) Image.load(sourceFile)) {
            // Thêm lớp điều chỉnh bản đồ Gradient.
...
**PSDJAVA-629. [Định dạng AI] Thêm hỗ trợ cho XPacket Metadata cho Định dạng AI**
...
{{< highlight java >}}
    ...
    static void assertIsNotNull(Object testObject) {
        if (testObject == null) {
            throw new RuntimeException("Các đối tượng thử nghiệm bằng null.");
        }
    }

{{< /highlight >}}

**PSDJAVA-630. Triển khai loại Warp Inflate, Squeeze và Twist**
...
**PSDJAVA-631. Chế độ Rgb và Lab không thể chứa ít hơn 3 kênh và nhiều hơn 4 kênh trong tập tin với Lớp ArtBoard**
...
{{< highlight java >}}
    ...
    try (PsdImage image = (PsdImage) Image.load(sourceFile)) {
        // Ở đây không nên xảy ra ngoại lệ
        image.save(outputFile);
    }

{{< /highlight >}}

**PSDJAVA-632. Khu vực xử lý phía trên phải là số dương. (Tham số 'areaToProcess') trong quá trình xử lý tập tin cụ thể**
...
{{< highlight java >}}
    ...
    try (PsdImage image = (PsdImage) PsdImage.load(sourceFile, psdLoadOptions)) {
        image.save(outputFile);
        // Không nên xảy ra ngoại lệ
    }

{{< /highlight >}}

**PSDJAVA-633. Hình ảnh mở rộng trên bảng vẽ bị cắt sau khi lưu. Dữ liệu bị mất nhưng xem trước đúng**
