---
title: Aspose.PSD pro .NET 21.4 - Poznámky k vydání
type: docs
weight: 90
url: /cs/net/aspose-psd-pro-net-21-4-poznamky-k-vydani/
---

{{% alert color="primary" %}} 

Tato stránka obsahuje poznámky k vydání pro [Aspose.PSD pro .NET 21.4](https://www.nuget.org/packages/Aspose.PSD/)

{{% /alert %}} 

|**Klíč**|**Souhrn**|**Kategorie**|
| :- | :- | :- |
|PSDNET-259|Renderování efektu vrstvy vzoru|Funkce|
|PSDNET-861|Přidání neznámých vlastností zdroje Vogk pro podporu změny velikosti vrstev tvarů s vektorovými cestami na obrázku PSD|Funkce|
|PSDNET-90|PSD není správně převeden na PDF|Chyba|
|PSDNET-830|Výjimka při ukládání konkrétního souboru PSD se textovými vrstvami|Chyba| (Opraveno dříve)
...
**PSDNET-90. PSD není správně převeden na PDF**
...
**Reprezentace efektu vrstvy vzoru**
...
**PSDNET-861. Přidání neznámých vlastností zdroje Vogk pro podporu změny velikosti vrstev tvarů s vektorovými cestami na obrázku PSD**

{{< highlight csharp >}}
            // Tento příklad ukazuje, jak získat a nastavit nové vlastnosti Transform a OriginBoxCorners
            // tvaru ShapeOriginSettings ve zdroji Vogk vrstvy FillLayer v souboru PSD
            ...
            using (PsdImage image = (PsdImage)Image.Load(sourceFileName)) 
            {
                ...
                // Uložení tohoto obrázku PSD s úpravami vlastností
                image.Save(outputPath, new PsdOptions(image));
            }

            // Načtení uloženého obrázku PSD s úpravami vlastností
            using (PsdImage image = (PsdImage)Image.Load(outputPath))
            {
                ...
                // Ověření, zda byly vlastnosti správně uloženy a načteny
                var setting = resource.ShapeOriginSettings[0];
                AssertAreEqual(true, setting.IsOriginIndexPresent);
                AssertAreEqual(false, setting.IsShapeInvalidatedPresent);
                AssertAreEqual(true, setting.IsOriginResolutionPresent);
            ...
            VogkResource GetVogkResource(FillLayer layer)
            {
                if (layer == null)
                {
                    throw new PsdImageArgumentException("Parametr vrstva by neměl být null.");
                }
                ...
                if (resource == null)
                {
                    throw new PsdImageException("VogkResource nenalezen.");
                }
            ...
            void AssertAreEqual(object actual, object expected)
            {
                ...
                {
                    throw new FormatException(string.Format("Aktuální hodnota {0} se neshoduje s očekávanou hodnotou {1}.", actual, expected));
                }
            }
{{< /highlight >}}