---
title: Aspose.PSD pro .NET 23.10 - Poznámky k vydání
type: docs
weight: 30
url: /cs/net/aspose-psd-pro-net-23-10-poznamky-k-vydani/
---

{{% alert color="primary" %}}

Tato stránka obsahuje poznámky k vydání pro [Aspose.PSD pro .NET 23.10](https://www.nuget.org/packages/Aspose.PSD/)

{{% /alert %}}

| **Klíč**     | **Souhrn**                                                                                                                | **Kategorie** |
|:------------|:------------------------------------------------------------------------|:--------|
| PSDNET-692 | Podpora svislého směru textu | Funkce |
| PSDNET-1402 | Použití nastavení obrysu ze zdroje vstk při vykreslování obrysu tvaru | Funkce |
| PSDNET-1607 | Implementace vykreslování vnitřní oblasti tvarů obrysu | Funkce |
| PSDNET-1644 | Nepřekreslovat vrstvu tvaru, pokud nebyla změněna | Funkce |
| PSDNET-1696 | [AI formát] Přidat podporu pro čtení hlavičky z PDF založených souborů AI | Funkce |
| PSDNET-1560 | Různé způsoby nastavení rozlišení souboru PSD nefungují | Chyba |
| PSDNET-1728 | FontSettings.SetFontsFolders nefunguje nebo Aspose.PSD používá nesprávný font | Chyba |
| PSDNET-1739 | Regrese. Oprava vyvolání výjimky Null reference při ukládání obrázku PSD s vrstvou tvaru | Chyba |
...
**PSDNET-692. Podpora svislého směru textu**
...
**PSDNET-1402. Použití nastavení obrysu ze zdroje vstk při vykreslování obrysu tvaru**
...
**PSDNET-1607. Implementace vykreslování vnitřní oblasti tvarů obrysu**
...
**PSDNET-1644. Nepřekreslovat vrstvu tvaru, pokud nebyla změněna**
...
using (PsdImage image = (PsdImage)Image.Load(sourceFile))
{
    ...
    // Neprovádět aktualizaci vrstvy tvaru před uložením

    image.Save(outputFile);

    // Zkontrolovat vykreslení uloženého souboru
}

// Zkontrolujte, že data vrstvy tvaru zůstávají nezměněna.
using (PsdImage image = (PsdImage)Image.Load(outputFile))
{
    ShapeLayer shapeLayer = (ShapeLayer)image.Layers[1];
    ColorFillSettings fillSettings = (ColorFillSettings)shapeLayer.Fill;
...
**PSDNET-1696. [AI formát] Přidat podporu pro čtení hlavičky z PDF založených souborů AI**
...
void AssertIsNotNull(object expected)
{
    if (expected == null)
    {
        throw new Exception("Objekt je null.");
    }
}
...
**PSDNET-1560. Různé způsoby nastavení rozlišení souboru PSD nefungují**
...
using (PsdImage newPsd = (PsdImage)new PsdImage(600, 400))
{
    PsdOptions ps     sovereignty olutions = new PsdOptions()
    {
...
**PSDNET-1728. FontSettings.SetFontsFolders nefunguje nebo Aspose.PSD používá nesprávný font**
...
**PSDNET-1739. Regrese. Oprava vyvolání výjimky Null reference při ukládání obrázku PSD s vrstvou tvaru**
...
{
    // Měla by se načíst bez výjimky

    ShapeLayer shapeLayer = (ShapeLayer)im.Layers[1];

    if (shapeLayer.RawDataSettings == null)
    {
        throw new Exception("RawDataSettings by neměla být null.");
    }
}
{{< /highlight >}}