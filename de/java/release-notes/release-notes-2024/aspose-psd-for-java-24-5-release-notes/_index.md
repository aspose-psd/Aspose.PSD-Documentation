---
title: Aspose.PSD für Java 24.5 - Versionshinweise
type: docs
weight: 40
url: /de/java/aspose-psd-fur-java-24-5-versionshinweise/
---

{{% alert color="primary" %}} Diese Seite enthält Versionshinweise für [Aspose.PSD für Java 24.5](https://downloads.aspose.com/psd/java/new-releases/aspose.psd-fur-java-24.5/) {{% /alert %}}

| **Schlüssel** | **Zusammenfassung**                                                                                                     | **Kategorie**  |
|:-------------|:------------------------------------------------------------------------------------------------------------------------|:--------------|
| PSDJAVA-617 | [AI-Format] Hinzufügen der Unterstützung für das Bearbeiten von AI-Dateien mit EPSF-Header                             | Funktion       |
| PSDJAVA-620 | Halbtransparente Verarbeitung falsch auf der Vorschau der PSD-Datei                                                  | Fehler           |
| PSDJAVA-621 | Rendern der Shape-Layer teilweise inkorrekt                                                                           | Fehler           |
| PSDJAVA-622 | Ausnahme beim Speichern von PSD-Dateien mit einer Größe von mehr als 200 MB und großen Dimensionen (Es gibt ein Problem beim Speicherverbrauch) | Fehler           |
| PSDJAVA-623 | Fehler beim Speichern des Bildes als Ausnahme beim Speichern in PDF nach dem Update von 23.7 auf 24.3               | Fehler           |
| PSDJAVA-624 | Beheben des Problems in der Methode GetFontInfoRecords für chinesische Schriften                                | Fehler           |
...
**PSDJAVA-617. [AI-Format] Hinzufügen der Unterstützung für das Bearbeiten von AI-Dateien mit EPSF-Header**
...
**PSDJAVA-620. Halbtransparente Verarbeitung falsch auf der Vorschau der PSD-Datei**
...
**PSDJAVA-621. Rendern der Shape-Layer teilweise inkorrekt**
...
{{< highlight java >}}
    ...
    public static void main(String[] args) {
        ...
        try (PsdImage im = (PsdImage) Image.load(sourceFile)) {
            ...
            // Ändern der Ebeneneigenschaften
            ...
            secondBezierKnotRecord.setPoints(new Point[]{
                    ...
                    pointFToResourcePoint(
                            new PointF(100, 490),
                            shapeLayer.getContainer().getSize()), // Ankerpunkt
                    pointFToResourcePoint(
                            new PointF(150, 490),
                            shapeLayer.getContainer().getSize())
            });
...
**PSDJAVA-622. Ausnahme beim Speichern von PSD-Dateien mit einer Größe von mehr als 200 MB und großen Dimensionen (Es gibt ein Problem beim Speicherverbrauch)**
...
**PSDJAVA-623. Fehler beim Speichern des Bildes als Ausnahme beim Speichern in PDF nach dem Update von 23.7 auf 24.3**
...
**PSDJAVA-624. Beheben des Problems in der Methode GetFontInfoRecords für chinesische Schriften**
            ...
            for (Layer layer : image.getLayers()) {
                if (layer instanceof TextLayer) {
                    ...
                    if ("best".equals(textLayer.getText())) {
                        // Ohne diese Korrektur hier wird es eine Ausnahme geben wegen der chinesischen Schriftart
