---
title: Aspose.PSD для Java 24.6 - Примечания к выпуску
type: docs
weight: 40
url: /ru/java/aspose-psd-for-java-24-6-release-notes/
---

{{% alert color="primary" %}} На этой странице содержатся заметки о релизе для [Aspose.PSD для Java 24.6](https://downloads.aspose.com/psd/java/new-releases/aspose.psd-for-java-24.6/) {{% /alert %}}

| **Ключ**    | **Краткое содержание**                                                                                           | **Категория** |
|:------------|:-----------------------------------------------------------------------------------------------------------------|:-------------|
| PSDJAVA-628 | Реализация поддержки слоя градиентной карты                                                                      | Функциональные возможности |
| PSDJAVA-629 | [Формат AI] Добавлена поддержка метаданных XPacket в формат AI                                                     | Функциональные возможности |
| PSDJAVA-630 | Реализация типов перемещения Inflate, Squeeze и Twist                                                              | Функциональные возможности |
| PSDJAVA-631 | Режимы Rgb и Lab не могут содержать менее 3 каналов и более 4 каналов в файле с слоями ArtBoard                | Ошибка |
| PSDJAVA-632 | Область обработки сверху должна быть положительной (Параметр 'areaToProcess') при обработке конкретного файла  | Ошибка |
| PSDJAVA-633 | Расширенное изображение за пределами холста обрезается после сохранения. Данные потеряны, но предварительный просмотр выглядит правильно | Ошибка |
...
**PSDJAVA-628. Реализация поддержки слоя градиентной карты**
...
{{< highlight java >}}
    ...
    public static void main(String[] args) {
        ...
        try (PsdImage im = (PsdImage) Image.load(sourceFile)) {
            // Добавляем слой коррекции градиента карты.
        ...
        // Проверяем сохраненные изменения.
...
**PSDJAVA-629. [Формат AI] Добавлена поддержка метаданных XPacket в формат AI**
...
{{< highlight java >}}
    ...
    static void assertIsNotNull(Object testObject) {
        if (testObject == null) {
            throw new RuntimeException("Тестовый объект равен нулю.");
        }
    }

{{< /highlight >}}

**PSDJAVA-630. Реализация типов перемещения Inflate, Squeeze и Twist**
...
**PSDJAVA-631. Режимы Rgb и Lab не могут содержать менее 3 каналов и более 4 каналов в файле с слоями ArtBoard**
...
**PSDJAVA-632. Область обработки сверху должна быть положительной. (Параметр 'areaToProcess') при обработке конкретного файла**
...
{{< highlight java >}}
    ...
    try (PsdImage image = (PsdImage) PsdImage.load(sourceFile, psdLoadOptions)) {
        image.save(outputFile);
        // Не должно возникнуть исключение
    }

{{< /highlight >}}

**PSDJAVA-633. Расширенное изображение за пределами холста обрезается после сохранения. Данные потеряны, но предварительный просмотр выглядит правильно**
