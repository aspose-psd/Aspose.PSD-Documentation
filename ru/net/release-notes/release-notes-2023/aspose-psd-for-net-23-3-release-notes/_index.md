---
title: Наши Примечания - Aspose.PSD для .NET 23.3
type: docs
weight: 100
url: /ru/net/aspose-psd-for-net-23-3-release-notes/
---

{{% alert color="primary" %}}

Эта страница содержит примечания к версии [Aspose.PSD для .NET 23.3](https://www.nuget.org/packages/Aspose.PSD/)

{{% /alert %}}

| **Ключ** | **Описание** | **Категория** |
| :- | :- | :- |
|PSDNET-146|Поддержка слоя коррекции Posterize|Функция|
|PSDNET-1366|Реализация поддержки VscgResource|Функция|
|PSDNET-1437|Добавлена поддержка ресурса PostResource для данных слоя коррекции Posterize|Функция|
|PSDNET-931|Неправильный экспорт слоя в PNG с кривыми коррекциями и режимом цвета CMYK|Ошибка|
|PSDNET-1403|Стиль абзаца пропадает после сохранения файла и его обновления через PS|Ошибка|
|PSDNET-1453|Нарушен вывод эффектов свечения и тени на тексте|Ошибка|
...
**PSDNET-146. Поддержка слоя коррекции Posterize**
...
**PSDNET-931. Неправильный экспорт в PNG слоя с кривыми коррекциями и режимом цвета CMYK**
...
{
    image.Save(outputFilePsd, new PsdOptions()); //', new PsdOptions()' провоцирует ошибку.
    image.Save(outputFile, new PngOptions() { ColorType = PngColorType.TruecolorWithAlpha });
}
{{< /highlight >}}

**PSDNET-1366. Реализация поддержки VscgResource**
...
using (PsdImage image = (PsdImage)Image.Load(sourceFile))
{
    ...
    ((DoubleStructure)rgbColorStructure.Structures[0]).Value = 255d; // Красный
    ((DoubleStructure)rgbColorStructure.Structures[1]).Value = 0d; // Зеленый
    ((DoubleStructure)rgbColorStructure.Structures[2]).Value = 0d; // Синий
...
**PSDNET-1403. Стиль абзаца пропадает после сохранения файла и его обновления через PS**
...
using (PsdImage image = (PsdImage)Aspose.PSD.Image.Load(sourceFile))
{
    ...
    // Добавление новых порций текста
...
using (PsdImage psdImage = (PsdImage)Aspose.PSD.Image.Load(outputFile))
{
    ...
    string key0 = @" >> /6 0 >> >> /1 "; // префикс к длине абзаца 0
    string key1 = @" >> /6 1 >> >> /1 "; // префикс к длине абзаца 1
    ...
    {
        throw new Exception(
            $"Длина абзацев неверна.\nОжидаемая: {expected0} и {expected1}\nРезультат: {lenPar0} и {lenPar1}\n");
    }
}
{{< /highlight >}}

**PSDNET-1437. Добавить поддержку ресурса PostResource для данных слоя коррекции Posterize**
...
**PSDNET-1453. Нарушен вывод эффектов свечения и тени на тексте**
...
using (var psdImage = new PsdImage(500, 500))
{
    ...
    // Добавить эффекты
    OuterGlowEffect glow = textLayer1.BlendingOptions.AddOuterGlow(); // исправить
    glow.Spread = 15;
    ((ColorFillSettings)glow.FillColor).Color = Color.Red;

    var shadow = textLayer2.BlendingOptions.AddDropShadow(); // исправить с длинным текстом
    shadow.Distance = 25;
    shadow.Color = Color.DarkBlue;

    var innerShadow = textLayer3.BlendingOptions.AddInnerShadow(); // исправить с длинным текстом
