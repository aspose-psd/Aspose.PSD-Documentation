---
title: Aspose.PSD voor .NET 22.8 - Release-opmerkingen
type: docs
weight: 50
url: /nl/net/aspose-psd-voor-net-22-8-release-opmerkingen/
---

{{% alert color="primary" %}}

Deze pagina bevat release-opmerkingen voor [Aspose.PSD voor .NET 22.8](https://www.nuget.org/packages/Aspose.PSD/)

{{% /alert %}}

|**Sleutel**|**Samenvatting**|**Categorie**|
| :- | :- | :- |
|PSDNET-1225|Onderzoeken en oplossen van problemen in de installer|Verbetering|
|PSDNET-800|Ondersteuning van Frame TimeLine vanuit PSD-bestand|Functie|
|PSDNET-1219|Ondersteuning van 'mlst' resource die voorkomt in ShmdResource als een sub-resource|Functie|
|PSDNET-814|Hash van Layer verandert als we layer.BlendingOptions.Effects aanroepen|Fout|
...
**PSDNET-800. Ondersteuning van Frame TimeLine vanuit PSD-bestand**
...
using (PsdImage psdImage = (PsdImage)Image.Load(sourceFile))
{
    ...
    // Wijzig dispose-methode van frame 1
    timeLine.Frames[0].DisposalMethod = FrameDisposalMethod.DoNotDispose;

    // Wijzig delay van frame 2
    timeLine.Frames[1].Delay = 15;

    // Wijzig opacity van 'Layer 1' op frame 2
    LayerState layerState11 = timeLine.Frames[1].LayerStates[timeLine.LayerIds[1]];
    layerState11.Opacity = 50;

    // Verplaats 'Layer 1' naar linksonderhoek op frame 3
    LayerState layerState21 = timeLine.Frames[2].LayerStates[timeLine.LayerIds[1]];
    layerState21.Offset = new Point(-50, 230);

    // Voeg nieuw frame toe
    List<Frame> frames = new List<Frame>(timeLine.Frames);
    frames.Add(new Frame(timeLine));
    timeLine.Frames = frames.ToArray();

    // Wijzig blendMode van 'Layer 1' op frame 4
    LayerState layerState31 = timeLine.Frames[3].LayerStates[timeLine.LayerIds[1]];
    layerState31.BlendMode = BlendMode.Dissolve;

    // Pas wijzigingen toe op PsdImage-instantie
    timeLine.ApplyTo(psdImage);
    psdImage.Save(outputPsd);
}
{{< /highlight >}}

**PSDNET-814. Hash van Layer verandert als we layer.BlendingOptions.Effects aanroepen**
...
using (var image = (PsdImage)Image.Load(sourceFile))
{
    ...
    {
        throw new Exception("Hash mag niet worden gewijzigd");
    }
}
{{< /highlight >}}

**PSDNET-1219. Ondersteuning van 'mlst' resource die voorkomt in ShmdResource als een sub-resource**
...
using (PsdImage image = (PsdImage)Image.Load(sourceFile))
{
    ...
    // Schakel layer 1 uit op frame 1
