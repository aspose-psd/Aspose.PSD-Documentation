---
title: Catatan Rilis Aspose.PSD untuk .NET 20.11
type: docs
weight: 20
url: /id/net/aspose-psd-untuk-net-20-11-catatan-rilis/
---

{{% alert color="primary" %}} 

Halaman ini berisi catatan rilis untuk [Aspose.PSD untuk .NET 20.11](https://www.nuget.org/packages/Aspose.PSD/)

{{% /alert %}} 

|**Kunci**|**Ringkasan**|**Kategori**|
| :- | :- | :- |
|PSDNET-713|Aspose.PSD tidak dapat mengonversi PSD ke Mode Warna/BitDepth lain tanpa menyimpannya sebagai file|Fitur|
|PSDNET-754|Menambahkan kemampuan untuk menyalin Layer Objek Pintar dalam gambar PSD|Fitur|
|PSDNET-267|Exception saat memuat dan menyimpan file PSD dengan Efek Layer|Bug|
|PSDNET-579|Metode "Image.LoadRawData" menimbulkan pengecualian NullPointer|Bug|
|PSDNET-741|ImageLoadException dilemparkan saat mencoba membuka file|Bug|
|PSDNET-744|Aspose.PSD 20.10: Tidak dapat memuat Psd|Bug|
...
## **Contoh Penggunaan:**
**PSDNET-267. Exception saat memuat dan menyimpan file PSD dengan Efek Layer**
{{< highlight csharp >}}
...
{{< /highlight >}}
**PSDNET-579. Metode "Image.LoadRawData" menimbulkan pengecualian NullPointer**
{{< highlight csharp >}}
...
{{< /highlight >}}
**PSDNET-713. Aspose.PSD tidak dapat mengonversi PSD ke Mode Warna/BitDepth lain tanpa menyimpannya sebagai file**
{{< highlight csharp >}}
            string dataDir = "PSDNET713_1\\";
            string outputDir = dataDir + "output\\";

            // Contoh-contoh berikut menunjukkan konversi format gambar PSD ke Mode Warna/BitDepth lain.
            ...
            // Menyimpan ke PSD kemudian memuatnya dan menyimpan ke PNG.
...
{{< /highlight >}}
**PSDNET-741. ImageLoadException dilemparkan saat mencoba membuka file**
{{< highlight csharp >}}
...
{{< /highlight >}}
**PSDNET-744. Aspose.PSD 20.10: Tidak dapat memuat Psd**
{{< highlight csharp >}}
...
{{< /highlight >}}
**PSDNET-754. Menambahkan kemampuan untuk menyalin Layer Objek Pintar dalam gambar PSD**
{{< highlight csharp >}}
            string dataDir = "PSDNET754_1\\";
            string outputDir = dataDir + "output\\";

            // Contoh ini menunjukkan bagaimana menyalin lapisan objek pintar dalam gambar PSD.
            ContohPenyalinanLayerObjekPintar("r-embedded-psd");
            ContohPenyalinanLayerObjekPintar("r-embedded-png");
            ContohPenyalinanLayerObjekPintar("r-embedded-transform");
            ContohPenyalinanLayerObjekPintar("new_panama-papers-8-trans4");

            void ContohPenyalinanLayerObjekPintar(string fileName)
            {
                ...
                using (PsdImage image = (PsdImage)Image.Load(filePath))
                {
                    ...
                    {
                        // Mari kita balik gambar objek pintar tersemat (untuk gambar PSD dalamnya kita hanya membalik lapisannya)
                        InvertImage(innerImage);

                        // Mari kita ganti gambar objek pintar tersemat dalam lapisan PSD
                        smartObjectLayer.ReplaceContents(innerImage);
                    }

                    // Lapisan duplikat berbagi gambar tersematnya dengan objek pintar asli
                    // dan harus diperbarui dengan eksplisit karena cache rendernya tetap tidak berubah.
                    // Kita update setiap objek pintar untuk memastikan bahwa lapisan baru yang dibuat oleh NewSmartObjectViaCopy
            ...
            void AssertIsTrue(bool condition)
            {
                if (!condition)
                {
                    throw new FormatException(string.Format("Diantisipasi benar"));
                }
            }
{{< /highlight >}}