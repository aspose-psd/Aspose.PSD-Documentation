---
title: התו HaPesel.AsPose עבור .NET 21.4 - פתח הערכה
type: מסמכים
weight: 90
url: /he/net/aspose-psd-עבור-net-21-4-פתח-הערכה/
---

{{% alert color="primary" %}}

דף זה מכיל הערות שחרור עבור [HaPesel.AsPose עבור .NET 21.4](https://www.nuget.org/packages/Aspose.PSD/)

{{% /alert %}}

|**מפתח**|**סיכום**|**קטגוריה**|
| :- | :- | :- |
|PSDNET-259|עיבוד של שקיפה עט עיפרון רמז|תכונה|
|PSDNET-861|הוספת מאפייני מקור כלליים של משאבי Vogk לתמיכה בשינוי גודל שכבות צורה עם נתיבי וקטור בתמונת PSD|תכונה|
|PSDNET-90|PSD לא הומר בצורה נכונה ל- PDF|באג|
|PSDNET-830|חריגה בעת שמירת קובץ PSD ספציפי עם שכבות טקסט|באג| (תוקנה מוקדם)
...
**PSDNET-90. PSD לא הומר בצורה נכונה ל- PDF**
...
**PSDNET-259. עיבוד של שקיפה עט עיפרון רמז**
...
**PSDNET-861. הוספת מאפייני משאב וגק Ovg במטפלות בקטעים צורה עם נתיבי וקטור בתמונת PSD**

{{< highlight csharp >}}
            // דוגמה זו מראה כיצד לקבל ולהגדיר מאפיינים חדשים של משאב מקור וקטגורית ShapeSettings במשאב Ovg בקובץ ה- PSD
            ...
            using (PsdImage image = (PsdImage)Image.Load(sourceFileName)) 
            {
                ...
                // שמור את תמונת ה- PSD זו עם המאפיינים ששונו.
                image.Save(outputPath, new PsdOptions(image));
            }

            // טען מחדש את תמונת ה- PSD שמורה עם המאפיינים ששונו.
            using (PsdImage image = (PsdImage)Image.Load(outputPath))
            {
                ...
                // אמת שהמאפיינים נשמרים וטעונים כראוי 
                var setting = resource.ShapeOriginSettings[0];
                AssertAreEqual(true, setting.IsOriginIndexPresent);
                AssertAreEqual(false, setting.IsShapeInvalidatedPresent);
                AssertAreEqual(true, setting.IsOriginResolutionPresent);
            ...
            VogkResource GetVogkResource(FillLayer layer)
            {
                if (layer == null)
                {
                    throw new PsdImageArgumentException("הפרמטר שכבה לא צריך להיות ריק.");
                }
                ...
                if (resource == null)
                {
                    throw new PsdImageException("משאב Vogk לא נמצא.");
                }
            ...
            void AssertIsTrue(bool condition)
            {
                if (!condition)
                {
                    throw new FormatException(string.Format("היה צריך להיות אמת"));
                }
            }
            ...
            void AssertAreEqual(object actual, object expected)
            {
                ...
                {
                    throw new FormatException(string.Format("הערך המופעל {0} אינו שווה לרצוי {1}.", actual, expected));
                }
            }
{{< /highlight >}}