---
title: Aspose.PSD for .NET 21.4 - Notatki dotyczące wydania
type: docs
weight: 90
url: /pl/net/aspose-psd-for-net-21-4-release-notes/
---

{{% alert color="primary" %}} 

Ta strona zawiera notatki dotyczące wydania [Aspose.PSD for .NET 21.4](https://www.nuget.org/packages/Aspose.PSD/)

{{% /alert %}} 

|**Klucz**|**Podsumowanie**|**Kategoria**|
| :- | :- | :- |
|PSDNET-259|Renderowanie efektu nakładki wzoru warstwy|Funkcja|
|PSDNET-861|Dodaj nieznane właściwości zasobów Vogk w celu obsługi zmiany rozmiaru warstw kształtu z ścieżkami wektorowymi w obrazie PSD|Funkcja|
|PSDNET-90|PSD nieprawidłowo konwertowany do pliku PDF|Błąd|
|PSDNET-830|Wyjątek podczas zapisywania określonego pliku PSD z warstwami tekstowymi|Błąd (Naprawiono wcześniej)
...
**PSDNET-90. PSD nieprawidłowo konwertowany do pliku PDF**
...
**PSDNET-259. Renderowanie efektu nakładki wzoru warstwy**
...
**PSDNET-861. Dodaj nieznane właściwości zasobów Vogk w celu obsługi zmiany rozmiaru warstw kształtu ze ścieżkami wektorowymi w obrazie PSD**

{{< highlight csharp >}}
            // Ten przykład pokazuje, jak pobrać i ustawić nowe właściwości Transform i OriginBoxCorners
            // ShapeOriginSettings w zasobie Vogk na warstwie FillLayer w pliku PSD
            ...
            using (PsdImage image = (PsdImage)Image.Load(sourceFileName)) 
            {
                ...
                // Zapisz ten obraz PSD z zmienionymi właściwościami.
                image.Save(outputPath, new PsdOptions(image));
            }

            // Załaduj zapisany obraz PSD z zmienionymi właściwościami.
            using (PsdImage image = (PsdImage)Image.Load(outputPath))
            {
                ...
                // Sprawdź, czy właściwości zostały poprawnie zapisane i załadowane
                var setting = resource.ShapeOriginSettings[0];
                AssertAreEqual(true, setting.IsOriginIndexPresent);
                AssertAreEqual(false, setting.IsShapeInvalidatedPresent);
                AssertAreEqual(true, setting.IsOriginResolutionPresent);
            ...
            VogkResource GetVogkResource(FillLayer layer)
            {
                if (layer == null)
                {
                    throw new PsdImageArgumentException("Parametr warstwy nie powinien być wartością null.");
                }
                ...
                if (resource == null)
                {
                    throw new PsdImageException("Nie znaleziono zasobu VogkResource.");
                }
            ...
            void AssertIsTrue(bool condition)
            {
                if (!condition)
                {
                    throw new FormatException(string.Format("Oczekiwane było true"));
                }
            }
            ...
            void AssertAreEqual(object actual, object expected)
            {
                ...
                {
                    throw new FormatException(string.Format("Wartość rzeczywista {0} nie jest równa wartości oczekiwanej {1}.", actual, expected));
                }
            }
{{< /highlight >}}